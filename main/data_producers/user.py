...
from config import USER_REGISTRATION_ENDPOINT, USERS_ENDPOINT
from utils import random_user_id, get_header, random_username, \
...
def create_admin_login():
    ...
    """
    if not os.path.exists('main/logindata.json'):
        with open('main/logindata.json', 'w', encoding='utf-8') as file:
            file.write("{}")
            logger.info("creating logindata.json file")
    else:
        logger.info("found logindata.json")
